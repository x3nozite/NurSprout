document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateNutritionList(log) {
    ...
    Object.keys(totalNutrition).forEach((nutrient) => {
      ...
      // update bar
      const nutrientElement = document.querySelector(
        `.nutrient[data-nutrient="${nutrient}"]`
      );
      if (nutrientElement) {
        const barFill = nutrientElement.querySelector(".bar-fill");
        const barText = nutrientElement.querySelector(
          ".top-row span:nth-child(2)"
        );

        barFill.style.width = `${percentage}%`;

        barText.textContent = `${total.toFixed(1)} / ${target} ${
          nutrient === "iron" || nutrient === "zinc" || nutrient === "vitaminC"
            ? "mg"
            : nutrient === "vitaminA"
            ? "mcg"
            : "g"
        ...
        const percentageText = nutrientElement.querySelector(
  ...
  function handleFoodLog() {
    ...
    //handles valid food
    const validFood = ["banana", "avocado", "pumpkin"];
